...
def website_temperature(main_page):
        ...
        main_page.page.wait_for_timeout(200)
    ...
    def test_comparison_internal_api (self, website_temperature):
        with allure.step("Получаем прогноз от внутреннего api"):
            response = requests.get(
                f'https://api.openweathermap.org/data/2.5/weather?lat=53.902284&lon=27.561831&units=metric&mode=json&appid={os.environ.get("OWM_API_KEY")}'
                )
