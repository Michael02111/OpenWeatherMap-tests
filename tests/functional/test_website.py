...
class TestOpenWeather(unittest.TestCase):
    ...
    def test_blog_page (self):
        ...
        with self.browser.contexts[0].expect_page() as new_page_info:
            blog_page_button.click() # Opens a new tab
            new_page = new_page_info.value
    ...
    def test_for_business_page (self):
        ...
        with self.browser.contexts[0].expect_page() as new_page_info:
            for_business_page_button.click() # Opens a new tab
            new_page = new_page_info.value
    ...
    def test_support_page_ask_questions (self):
        ...
        with self.browser.contexts[0].expect_page() as new_page_info:
            support_field.click() # Opens a new tab
            new_page = new_page_info.value
...
#Проверка элементов формы входа в аккаунт
    ...
    ####Тест на проверку возможности удаления города из списка отслеживаемых городов

    # #### Тест на проверку текущей даты

   
    ####Тест на проверку возможности получения прогноза погоды на ближайшие дни для выбранного города


    ####Тест на проверку возможности получения прогноза погоды на ближайшие часы для выбранного города


    #### Тест на проверку возможности получения информации о качестве воздуха для выбранного города


    ####Тест на проверку возможности получения информации о ветре для выбранного города
    
    ####Тест на проверку возможности удаления города из списка отслеживаемых городов



    ####Тест на проверку возможности получения информации о влажности для выбранного города
    

    ####Тест на проверку возможности получения информации о давлении для выбранного города
